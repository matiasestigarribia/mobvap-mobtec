...
```Here is the **QA/Tester Playwright System Prompt** tailored for the **MOBVAP** platform. It follows the exact structure and length of your example, translated to English and adapted to the specific context of your project (Science Fair, Media Galleries, Comments, etc.).

Save this file as `agent_qa.md`.

---

# QA/Tester Playwright - MOBVAP Edition

## Identity

You are a **Lead Quality Assurance Engineer** specializing in End-to-End (E2E) testing using **Playwright**. Your mission is to validate the refactoring work done by the Frontend Agent on the MOBVAP-MOBTEC platform. You ensure the new "High-End Science Fair" aesthetic is implemented correctly without breaking existing functionality (Galleries, Comments, Auth). You use the **Playwright MCP Server** to navigate, interact, and validate the system from a user's perspective.

## Stack & Tools

* **Playwright MCP Server** (Navigation, Interaction, Screenshots)
* **Python 3.11+** (For test scripts if needed)
* **HTML/DOM Knowledge** (For robust CSS/XPath selectors)
* **Visual Regression Principles** (Design System validation)
* **Mobile Responsiveness Standards** (Viewport testing)

## Responsibilities

### 1. Visual Integrity & Design System

* Validate that the new UI matches the **High-End Academic** aesthetic.
* Verify color palette usage (Blue-700, Slate-50, etc.) matches the `agent_frontend` specifications.
* Check typography hierarchy (Headings vs Body).
* **Critical:** Verify that "Hero Sections" and "Cards" are rendered correctly.

### 2. Functional E2E Testing

* Ensure the visual refactor did not break business logic.
* Test the **Media Pipeline**: Do images load? Do video thumbnails appear?
* Test **Interactivity**: Does the mobile menu open? Do forms submit?
* Test **Navigation**: Are links to Editions/Projects working?

### 3. Responsiveness (Mobile First)

* Test the layout on Mobile (iPhone SE/12), Tablet (iPad), and Desktop.
* Ensure no horizontal scrolling on mobile.
* Verify touch targets (buttons) are accessible.

### 4. Bug Detection & Reporting

* Detect JavaScript errors introduced by Alpine.js.
* Identify broken images (404s) or layout shifts (CLS).
* Report bugs with clear severity levels and reproduction steps.
...
* `url`: Target URL (e.g., `http://localhost:8000`)
* `width`: 375 (Mobile) | 768 (Tablet) | 1280 (Desktop)
* `height`: Viewport height

**Visual Evidence**
`mcp__playwright__playwright_screenshot`

* `name`: Descriptive filename (e.g., `home_mobile_v2.png`)
* `fullPage`: `true` (Essential for long gallery pages)

**Interaction**
`mcp__playwright__playwright_click`

* `selector`: Robust CSS selector (e.g., `button[type="submit"]`)
...
* Verifies content presence (e.g., "Welcome to MOBVAP").

`mcp__playwright__playwright_console_logs`

* `type`: 'error' (Crucial for catching Alpine.js syntax errors).
...
**TC001: Hero Section & Branding**

* **Goal:** Validate the "High-End" first impression.
* **Steps:**
1. Navigate to `/` (Desktop).
2. Check for main Logos (UNIP/CIEBP).
3. Verify the Hero Title is visible and styled (`text-3xl+`).
4. Capture Screenshot: `home_desktop.png`.


* **Expected:** No broken images, correct gradient backgrounds, logos present.

**TC002: Mobile Navigation**

* **Goal:** Test Alpine.js hamburger menu.
* **Steps:**
1. Set Viewport: 375x667.
2. Navigate to `/`.
3. Click the "Menu" button (Hamburger).
4. Verify links (Projects, About, Login) are visible.
5. Capture Screenshot: `home_mobile_menu.png`.


* **Expected:** Menu expands smoothly, links are clickable, no layout overlap.

### 🖼️ Gallery & Projects

**TC003: Media Grid Rendering**

* **Goal:** Ensure the masonry/grid layout works.
* **Steps:**
1. Navigate to `/editions/2025/photos`.
2. Wait for images to load.
3. Check if grid columns are aligned (3 cols desktop / 1 col mobile).
4. Capture Screenshot: `gallery_grid.png`.


* **Expected:** Images have rounded corners, shadows (hover effect), and no overlapping text.

**TC004: Pagination UI**

* **Goal:** Verify pagination controls are styled.
* **Steps:**
1. Scroll to bottom of Gallery.
2. Check "Next/Previous" buttons styling (should be Tailwind buttons, not raw HTML links).
3. Click "Next".


* **Expected:** Navigates to page 2, styles remain consistent.

### 💬 Interaction (Comments)

**TC005: Post Comment (UI Feedback)**

* **Goal:** Validate the form styling and loading state.
* **Steps:**
1. Navigate to a Project Detail page.
2. Fill `Name` and `Comment`.
3. Click "Submit".
4. **Critical:** Check for "Loading Spinner" (Visual check).
5. Check for Success Message (Toast/Banner).
6. Capture Screenshot: `comment_success.png`.


* **Expected:** Form inputs have focus rings, button shows hover state, success message is green/emerald.

### 🔐 Authentication Pages

**TC006: Login Page Aesthetic**

* **Goal:** Ensure Login looks professional (not default Django).
* **Steps:**
1. Navigate to `/login/`.
2. Verify centered card layout.
3. Check input field styling (borders, padding).
4. Capture Screenshot: `login_page.png`.


* **Expected:** Clean card centered on screen, "Sign In" button prominent.

---

## Execution Example: Full Smoke Test

**TEST: SMOKE-MOBVAP-01 - Visual & Functional Check**

**OBJECTIVE:** Validate the Frontend Agent's refactoring of the Homepage.

**DETAILED STEPS:**

1. **SETUP**
* `playwright_navigate`: `http://localhost:8000/` (Width: 1280)
* `playwright_screenshot`: `01_home_initial.png`


2. **CHECK CONSOLE**
* `playwright_console_logs`: type='error'
* *Check:* Are there any `Alpine Error: ...` or 404s?


3. **VALIDATE COMPONENTS**
* `playwright_get_visible_text`: Check for "MOBVAP" and "Science Fair".
* `playwright_click`: Selector for "Latest Projects" CTA.
* *Verify:* URL changes to `/projects`.


4. **MOBILE CHECK**
* `playwright_navigate`: `http://localhost:8000/` (Width: 375)
* `playwright_screenshot`: `02_home_mobile.png`
...
* ✓ Home loads with new "Slate/Blue" theme.
* ✓ No JS errors in console.
* ✓ CTA button works.
* ✓ Mobile view is stacked (column) and readable.

---

## Bug Report Template

**BUG-XXX: [Component Name] - [Short Description]**

**SEVERITY:** [Critical (Blocker) | High (Broken UI) | Medium (Visual Glitch) | Low (Polish)]

**DESCRIPTION:**
[Clear description of the visual or functional defect]

**REPRODUCTION STEPS:**

1. Navigate to `[URL]`
2. Set viewport to `[Width]`
3. Action performed `[Click/Scroll]`

**EXPECTED BEHAVIOR:**
[e.g., The card should hover with a shadow and remain aligned.]

**ACTUAL BEHAVIOR:**
[e.g., The card breaks onto a new line and the text overflows the container.]
...
* **❌ ERROR:** Modifying Python Code to fix a test.
* *Correction:* Only report the bug. The Frontend Agent must fix the HTML/CSS.


* **❌ ERROR:** Using brittle XPaths (`/html/body/div[2]/div[1]`).
* *Correction:* Use semantic selectors: `nav`, `footer`, or `.project-card`.


* **❌ ERROR:** Ignoring Console Errors.
* *Correction:* Alpine.js fails silently often. Always check `console_logs`.


* **❌ ERROR:** Testing on "Perfect Internet" only.
* *Correction:* Ensure loading skeletons or spinners appear (visually) during transitions.



## Resources

* **PRD:** `/PRD.md` (Check "Success Metrics")
* **Tasks:** `/TASKS.md` (Verify completed items)
* **Design System:** Defined in `agent_frontend.md`# Product Requirements Document (PRD) - Visual Overhaul
...
- "Wow Factor" increased on the Home Page.# Todo List for Visual Upgrade Agent

## Phase 1: Foundation
- [ ] **Setup:** Analyze current `base.html` and `tailwind.config.js`.
- [ ] **Typography:** Define global font family and sizes in base CSS.
- [ ] **Navbar:** Redesign the navigation bar to be responsive (hamburger menu on mobile) with Alpine.js.
- [ ] **Footer:** Create a professional footer with links, copyright, and social icons.

## Phase 2: Landing Page (Home)
- [ ] **Hero Section:** Design a full-width hero component with background image/gradient and main Call-to-Action.
- [ ] **Features Grid:** Style the "About" or "Highlights" section using a 3-column grid.
- [ ] **Stats/Numbers:** Create a visual component for stats (if applicable).

## Phase 3: Content Pages
- [ ] **Gallery:** Refactor the photo/video list into a masonry or clean grid layout.
- [ ] **Detail View:** Style the individual project/news page for better readability.

## Phase 4: Interactive Elements
- [ ] **Buttons:** Standardize all `<a>` and `<button>` classes across templates.
- [ ] **Forms:** Apply Tailwind classes to Django form fields (Input, Select, Textarea).
- [ ] **Alerts:** Style Django messages (Success/Error/Warning) as floating toasts or banners..dockerignore
.env.example
...
app/__pycache__/wsgi.cpython-311.pyc
app/asgi.py
app/settings.py
...
comments/__pycache__/__init__.cpython-311.pyc
comments/__pycache__/admin.cpython-311.pyc
comments/__pycache__/apps.cpython-311.pyc
comments/__pycache__/models.cpython-311.pyc
comments/__pycache__/serializers.cpython-311.pyc
comments/__pycache__/services.cpython-311.pyc
comments/__pycache__/signals.cpython-311.pyc
comments/__pycache__/urls.cpython-311.pyc
comments/__pycache__/views.cpython-311.pyc
comments/admin.py
comments/apps.py
comments/migrations/0001_initial.py
comments/migrations/__init__.py
comments/migrations/__pycache__/0001_initial.cpython-311.pyc
comments/migrations/__pycache__/__init__.cpython-311.pyc
comments/models.py
...
comments/tests/__pycache__/__init__.cpython-311.pyc
comments/tests/__pycache__/test_services.cpython-311.pyc
comments/tests/__pycache__/test_signals.cpython-311.pyc
comments/tests/test_services.py
comments/tests/test_signals.py
comments/urls.py
comments/views.py
docker-compose.yml
editions/__init__.py
editions/__pycache__/__init__.cpython-311.pyc
editions/__pycache__/admin.cpython-311.pyc
editions/__pycache__/apps.cpython-311.pyc
editions/__pycache__/models.cpython-311.pyc
editions/__pycache__/serializers.cpython-311.pyc
editions/__pycache__/urls.cpython-311.pyc
editions/__pycache__/views.cpython-311.pyc
editions/admin.py
editions/apps.py
editions/migrations/0001_initial.py
editions/migrations/0002_alter_edition_preview_image1_and_more.py
editions/migrations/0003_alter_edition_preview_image1_and_more.py
photos/migrations/__pycache__/0003_alter_photo_image_file.cpython-311.pyc
photos/migrations/__pycache__/__init__.cpython-311.pyc
photos/models.py
photos/serializers.py
photos/templates/photos.html
photos/tests/__init__.py
photos/tests/__pycache__/__init__.cpython-311.pyc
photos/tests/__pycache__/test_models.cpython-311.pyc
photos/tests/__pycache__/test_views.cpython-311.pyc
photos/tests/test_models.py
photos/tests/test_views.py
photos/views.py
requirements.txt
requirements_for_dev.txt
rulespagecontents/__init__.py
rulespagecontents/__pycache__/__init__.cpython-311.pyc
rulespagecontents/__pycache__/admin.cpython-311.pyc
rulespagecontents/__pycache__/apps.cpython-311.pyc
rulespagecontents/__pycache__/models.cpython-311.pyc
rulespagecontents/__pycache__/serializers.cpython-311.pyc
rulespagecontents/__pycache__/urls.cpython-311.pyc
rulespagecontents/__pycache__/views.cpython-311.pyc
rulespagecontents/admin.py
rulespagecontents/apps.py
rulespagecontents/migrations/0001_initial.py
rulespagecontents/migrations/0002_alter_rulespagecontent_options_and_more.py
rulespagecontents/migrations/__init__.py
rulespagecontents/migrations/__pycache__/0001_initial.cpython-311.pyc
rulespagecontents/migrations/__pycache__/0002_alter_rulespagecontent_options_and_more.cpython-311.pyc
rulespagecontents/migrations/__pycache__/__init__.cpython-311.pyc
rulespagecontents/models.py
rulespagecontents/serializers.py
rulespagecontents/templates/rules.html
rulespagecontents/urls.py
rulespagecontents/views.py
static/css/input.css
static/css/output.css
static/images/general/mobtec-logo.jpg
static/images/general/mobtec-logo1.png
static/images/general/mobvap-logo.jpg
static/images/general/mobvap-logo1.png
static/images/logos/apoio/logo_bse.png
static/images/logos/apoio/logo_cultivotech.png
static/images/logos/apoio/logo_dcta.png
static/images/logos/apoio/logo_dsa.png
static/images/logos/apoio/logo_galileo_galilei.png
static/images/logos/apoio/logo_instituto_digital.png
static/images/logos/apoio/logo_kamila_dias.png
static/images/logos/apoio/logo_marinha_do_brasil.png
static/images/logos/apoio/logo_mf.png
static/images/logos/apoio/logo_mg_acai.png
static/images/logos/apoio/logo_modirum.png
static/images/logos/apoio/logo_nunes.png
static/images/logos/apoio/logo_parque_tecnologico_santos.png
static/images/logos/apoio/logo_prefeitura_santos.png
static/images/logos/apoio/logo_quero_aprender.png
static/images/logos/apoio/logo_robo.png
static/images/logos/apoio/logo_sao_francisco.png
static/images/logos/apoio/logo_silvio_del_duca.png
static/images/logos/apoio/logo_sirvase_bem.png
static/images/logos/apoio/logo_teatro_sergio.png
static/images/logos/apoio/logo_thermas_do_vale.png
static/images/logos/apoio/logo_thiago_gama.png
static/images/logos/apoio/logo_troglio_sports.png
static/images/logos/apoio/logo_va.png
static/images/logos/apoio/logo_vincula_martech.png
static/images/logos/realizacao/logo_ciebp.png
static/images/logos/realizacao/logo_jc.png
static/images/logos/realizacao/logo_secretaria_da_educacao.png
static/images/logos/realizacao/logo_unip.png
static/images/logos/realizacao/logo_ure_sjc.png
staticfiles/admin/css/autocomplete.css
staticfiles/admin/css/autocomplete.d24f10bdee41.css
staticfiles/admin/css/base.96c479cedf7a.css
staticfiles/admin/css/base.css
staticfiles/admin/css/changelists.59465e72d1ef.css
staticfiles/admin/css/changelists.css
staticfiles/admin/css/dark_mode.1215cee25eaa.css
staticfiles/admin/css/dark_mode.css
staticfiles/admin/css/dashboard.css
staticfiles/admin/css/dashboard.e90f2068217b.css
staticfiles/admin/css/forms.ce1314886a7b.css
staticfiles/admin/css/forms.css
staticfiles/admin/css/login.a3b47c458e5d.css
staticfiles/admin/css/login.css
staticfiles/admin/css/nav_sidebar.css
staticfiles/admin/css/nav_sidebar.dd925738f4cc.css
staticfiles/admin/css/responsive.80b7f3c4f68f.css
staticfiles/admin/css/responsive.css
staticfiles/admin/css/responsive_rtl.011e68bec437.css
staticfiles/admin/css/responsive_rtl.css
staticfiles/admin/css/rtl.66af67f66f09.css
staticfiles/admin/css/rtl.css
staticfiles/admin/css/unusable_password_field.b433f2a95fba.css
...
staticfiles/admin/css/widgets.308c8f8831d6.css
staticfiles/admin/css/widgets.css
staticfiles/admin/img/LICENSE
staticfiles/admin/img/LICENSE.2c54f4e1ca1c
staticfiles/admin/img/README.9849248c9207.txt
staticfiles/admin/img/README.txt
staticfiles/admin/img/calendar-icons.93ab098d1ac1.svg
...
staticfiles/admin/img/icon-addlink.073aeb1feda7.svg
...
staticfiles/admin/img/icon-changelink.7eddb320e61f.svg
...
staticfiles/admin/img/inline-delete.358e965fe3e7.svg
...
staticfiles/admin/js/SelectFilter2.58388953117f.js
staticfiles/admin/js/SelectFilter2.js
staticfiles/admin/js/actions.f1d5653edb59.js
...
staticfiles/admin/js/admin/RelatedObjectLookups.ed6240809a40.js
...
staticfiles/admin/js/inlines.89b3c627c5dc.js
...
staticfiles/admin/js/popup_response.96190d343c22.js
...
staticfiles/admin/js/theme.91cf832f559e.js
staticfiles/admin/js/theme.js
staticfiles/admin/js/unusable_password_field.017ea86b6ae4.js
...
staticfiles/css/input.99c0b8c65d13.css
staticfiles/css/input.css
staticfiles/css/output.1272ee262005.css
staticfiles/css/output.css
staticfiles/images/general/mobtec-logo.9ca3e6992e8c.jpg
staticfiles/images/general/mobtec-logo.jpg
staticfiles/images/general/mobtec-logo1.png
staticfiles/images/general/mobvap-logo.00189400640a.jpg
staticfiles/images/general/mobvap-logo.jpg
staticfiles/images/general/mobvap-logo1.png
staticfiles/images/logos/apoio/logo_bse.6691fbffbebf.png
staticfiles/images/logos/apoio/logo_bse.8738910b861f.png
staticfiles/images/logos/apoio/logo_bse.png
staticfiles/images/logos/apoio/logo_cultivotech.4621c04be1cd.png
staticfiles/images/logos/apoio/logo_cultivotech.bee755d86890.png
staticfiles/images/logos/apoio/logo_cultivotech.png
staticfiles/images/logos/apoio/logo_dcta.3208afaf149f.png
staticfiles/images/logos/apoio/logo_dcta.png
staticfiles/images/logos/apoio/logo_dsa.d1e5cb5f6d8c.png
staticfiles/images/logos/apoio/logo_dsa.png
staticfiles/images/logos/apoio/logo_thermas_do_vale.703dd3d438db.png
staticfiles/images/logos/apoio/logo_thermas_do_vale.png
staticfiles/images/logos/apoio/logo_thiago_gama.ad2c05094eec.png
staticfiles/images/logos/apoio/logo_thiago_gama.b068f350a7f1.png
staticfiles/images/logos/apoio/logo_thiago_gama.png
staticfiles/images/logos/apoio/logo_troglio_sports.d9c4eb1c93b7.png
staticfiles/images/logos/apoio/logo_troglio_sports.png
staticfiles/images/logos/apoio/logo_va.71824ba5e1d0.png
staticfiles/images/logos/apoio/logo_va.png
staticfiles/images/logos/apoio/logo_vincula_martech.15e4f887b3da.png
staticfiles/images/logos/apoio/logo_vincula_martech.851765f05bb3.png
staticfiles/images/logos/apoio/logo_vincula_martech.png
staticfiles/images/logos/realizacao/logo_ciebp.ffc5641e5edf.png
staticfiles/images/logos/realizacao/logo_ciebp.png
staticfiles/images/logos/realizacao/logo_jc.b56bfea9bc0e.png
staticfiles/images/logos/realizacao/logo_jc.png
staticfiles/images/logos/realizacao/logo_secretaria_da_educacao.06e98eabd121.png
staticfiles/images/logos/realizacao/logo_secretaria_da_educacao.png
staticfiles/images/logos/realizacao/logo_unip.13338bc9da57.png
staticfiles/images/logos/realizacao/logo_unip.4507a2b0ec7c.png
staticfiles/images/logos/realizacao/logo_unip.png
staticfiles/images/logos/realizacao/logo_ure_sjc.b99ff9d32ca2.png
staticfiles/images/logos/realizacao/logo_ure_sjc.png
staticfiles/staticfiles.json
tailwind.config.js
videos/__init__.py
videos/__pycache__/__init__.cpython-311.pyc
videos/__pycache__/admin.cpython-311.pyc
videos/__pycache__/apps.cpython-311.pyc
videos/__pycache__/models.cpython-311.pyc
videos/__pycache__/serializers.cpython-311.pyc
videos/__pycache__/services.cpython-311.pyc
videos/__pycache__/views.cpython-311.pyc
videos/admin.py
videos/apps.py
videos/migrations/0001_initial.py
videos/migrations/0002_alter_video_video_file.py
videos/migrations/0003_alter_video_video_file_alter_video_video_thumbnail.py
videos/migrations/0004_alter_video_video_thumbnail.py
videos/migrations/0005_alter_video_video_thumbnail.py
videos/migrations/0006_alter_video_video_title.py
videos/migrations/__init__.py
videos/migrations/__pycache__/0001_initial.cpython-311.pyc
videos/migrations/__pycache__/0002_alter_video_video_file.cpython-311.pyc
videos/migrations/__pycache__/0003_alter_video_video_file_alter_video_video_thumbnail.cpython-311.pyc
videos/migrations/__pycache__/0004_alter_video_video_thumbnail.cpython-311.pyc
videos/migrations/__pycache__/0005_alter_video_video_thumbnail.cpython-311.pyc
videos/migrations/__pycache__/0006_alter_video_video_title.cpython-311.pyc
videos/migrations/__pycache__/__init__.cpython-311.pyc
videos/models.py
videos/serializers.py
videos/services.py
videos/templates/videos.html
videos/tests/__init__.py
videos/tests/__pycache__/__init__.cpython-311.pyc
videos/tests/__pycache__/test_models.cpython-311.pyc
videos/tests/__pycache__/test_views.cpython-311.pyc
videos/tests/test_models.py
videos/tests/test_views.py
videos/views.py
